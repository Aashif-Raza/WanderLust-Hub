...
module.exports.apiFilterByCategory = async (req, res) => {
  const { category } = req.params;
  const formattedCategory =
    category.charAt(0).toUpperCase() + category.slice(1).toLowerCase();

  const listings = await Listing.find({ category: formattedCategory });
  res.json(listings); // return as JSON for frontend JS
};
...
module.exports.bookNow = async (req, res) => {
  ...
  req.flash("success", `Booking confirmed successfully, ${username}!`);
  res.redirect(`/listings/${id}`);
};
